...
function idsEqual(left, right) {
    return toComparableId(left) === toComparableId(right);
}
...
function renderFileTypeFilters() {
    ...
    const accept = accountFileTypes
        .flatMap((type) => (Array.isArray(type.extensions) ? type.extensions : []).map((ext) => `.${ext}`))
        .join(',');
    const inputIds = ['accountFilesInput', 'qc-accountFilesInput', 'pageAccountFilesInput'];
    inputIds.forEach((id) => {
...
export async function deleteComment(commentId) {
    ...
    const comment = accountFilesState.currentAccountData.comments.find(c => idsEqual(c.id, commentId));
    if (!comment) return;
    ...
    try {
        ...
        accountFilesState.currentAccountData.comments = accountFilesState.currentAccountData.comments.filter(c => !idsEqual(c.id, commentId));
        renderComments();
    } catch (error) {
...
export function enableEditMode(event, id) {
    ...
    const comment = accountFilesState.currentAccountData.comments.find(c => idsEqual(c.id, id));
    if (!comment) return;
...
export async function saveUpdateEdit(event, id) {
    ...
    const commentIndex = accountFilesState.currentAccountData.comments.findIndex(c => idsEqual(c.id, id));
...
export function renderFilesList(filteredFiles) {
    ...
    const html = accountFilesState.loading ? `
            <div class="empty-placeholder">
                <i class="fas fa-spinner fa-spin"></i>
                <p>Loading files...</p>
            </div>
...
export function previewFile(fileId) {
    const fileData = accountFilesState.currentAccountData.files.find(f => idsEqual(f.id, fileId));
    if (!fileData) return;
...
export async function deleteFile(fileId) {
    ...
    try {
        ...
        accountFilesState.currentAccountData.files = accountFilesState.currentAccountData.files.filter(f => !idsEqual(f.id, fileId));
        renderAccountFiles();
    } catch (error) {
...
function buildNoteSubjectOptions(selected = '') {
    const subjects = Array.isArray(accountFilesState.noteSubjects) && accountFilesState.noteSubjects.length > 0
        ? accountFilesState.noteSubjects
        : DEFAULT_NOTE_SUBJECTS;

    const options = ['<option value="">Select a subject...</option>'];
    subjects.forEach((subject) => {
        const isSelected = selected === subject;
        options.push(`<option value="${escapeHtml(subject)}" ${isSelected ? 'selected' : ''}>${escapeHtml(subject)}</option>`);
    });
...
// Save Named Note
export async function saveNamedNote(e) {
    e.preventDefault();
    ...
    try {
        const payload = await createAccountNoteApi(getCurrentAccountCode(), { subject, content });
        if (payload?.note) {
            accountFilesState.currentAccountData.notes.unshift(payload.note);
            renderNamedNotes();
            closeNoteModal();
        }
    } catch (error) {
...
export function renderNamedNotes() {
    ...
    const html = accountFilesState.currentAccountData.notes.length === 0 ? `
        ...
        <div class="compact-note-card" data-note-id="${note.id}">
            ...
            ` : ''}
            <h4 class="compact-note-title">${escapeHtml(note.subject || note.title)}</h4>
...
export async function deleteNote(noteId) {
    const note = accountFilesState.currentAccountData.notes.find(n => idsEqual(n.id, noteId));
    if (!note) return;
    ...
    const confirmDelete = confirm(`Are you sure you want to delete the note "${note.subject || note.title}"?`);
    if (!confirmDelete) return;

    try {
        await deleteAccountNoteApi(getCurrentAccountCode(), noteId);
        accountFilesState.currentAccountData.notes = accountFilesState.currentAccountData.notes.filter(n => !idsEqual(n.id, noteId));
        renderNamedNotes();
    } catch (error) {
...
export function viewNote(noteId) {
    ...
    modal.innerHTML = `
        <div class="note-modal-header">
            <h3 class="note-modal-title">${escapeHtml(note.subject || note.title)}</h3>
...
export function purgeAccountFilesData(accountId) {
    ...
    if (accountFilesState.currentAccountId === accountId) {
        ...
        accountFilesState.loading = false;
        accountFilesState.error = null;
        accountFilesState.activeRequestId += 1;
    }
...
export function previewModalFile(fileId) {
    if (!currentModalAccountId) return;
    const accountData = getAccountData(currentModalAccountId);
    const fileData = accountData.files.find(f => idsEqual(f.id, fileId));
    if (!fileData) return;
...
export function deleteModalFile(fileId) {
    ...
    accountData.files = accountData.files.filter(f => !idsEqual(f.id, fileId));
    saveAccountData();
    renderModalFiles(accountData.files);
}
...
export function viewModalNote(noteId) {
    if (!currentModalAccountId) return;
    const accountData = getAccountData(currentModalAccountId);
    const note = accountData.notes.find(n => idsEqual(n.id, noteId));
    if (!note) return;
...
export function deleteModalNote(noteId) {
    ...
    accountData.notes = accountData.notes.filter(n => !idsEqual(n.id, noteId));
    saveAccountData();
    renderModalNotes(accountData.notes);
}
...
export function deleteModalComment(commentId) {
    if (!currentModalAccountId) return;
    const accountData = getAccountData(currentModalAccountId);
    const comment = accountData.comments.find(c => idsEqual(c.id, commentId));
    if (!comment) return;
    ...
    accountData.comments = accountData.comments.filter(c => !idsEqual(c.id, commentId));
    saveAccountData();
    renderModalComments(accountData.comments);
}

// Window bridges removed - all functions are exported and imported where needed
